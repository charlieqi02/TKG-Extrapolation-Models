...
class RGCNEntityDataset(object):
    ...
    def load(self, bfs_level=2, relabel=False):
        ...
        # bfs to reduce edges
        if bfs_level > 0:
            logging.info("removing nodes that are more than {} hops away".format(bfs_level))
            ...
            if relabel:
                ...
                logging.info("{} nodes left".format(self.num_nodes))
        else:
...
class RGCNLinkDataset(object):
    ...
    def load(self, load_time=True):
        ...
        self.splits['train'] = np.array(_read_triplets_as_list(train_path, load_time))
        self.splits['valid'] = np.array(_read_triplets_as_list(valid_path, load_time))
        self.splits['test'] = np.array(_read_triplets_as_list(test_path, load_time))
        self.num_nodes = len(entity_dict)
        logging.info("# Sanity Check:  entities: {}".format(self.num_nodes))
        self.num_rels = len(relation_dict)
        self.relation_dict = relation_dict
        self.entity_dict = entity_dict
        logging.info("# Sanity Check:  relations: {}".format(self.num_rels))
        logging.info("# Sanity Check:  edges (facts in train set): {}".format(len(self.splits['train'])))
...
class RDFReader(object):
    ...
    def __init__(self, file):
        ...
        logging.info("Graph loaded, frequencies counted.")
...
def _load_data(dataset_str='aifb', dataset_path=None):
    ...
    logging.info('Loading dataset', dataset_str)
    ...
    if os.path.isfile(edge_file) and os.path.isfile(labels_file) and \
        ...
        logging.info('Number of nodes: ', num_node)
        logging.info('Number of edges: ', len(edge_list))
        logging.info('Number of relations: ', num_rel)

        labels = _load_sparse_csr(labels_file)
        labeled_nodes_idx = list(labels.nonzero()[0])

        logging.info('Number of classes: ', labels.shape[1])
        ...
        logging.info('{} classes: {}'.format(len(labels_set), labels_set))
        ...
        for nod, lab in zip(labels_train_df[nodes_header].values,
            ...
            if nod in nodes_u_dict:
                ...
                logging.info(u'Node not in dictionary, skipped: ',
